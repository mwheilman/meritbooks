...
export async function POST(request: Request) {
  ...
  if (body.step === 'chart_of_accounts') {
    ...
    try {
      const { templateAccounts, totalAccounts } = await seedChartOfAccounts(supabase, orgId);
      return NextResponse.json(
        { success: true, accountCount: totalAccounts, templateAccounts },
        { status: 201 },
      );
    } catch (e) {
      const msg = e instanceof Error ? e.message : 'COA seeding failed';
      console.error('[setup] chart_of_accounts:', msg);
