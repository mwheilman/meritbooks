...
/**
 * POST /api/reports/packs/[id]/pdf — run a SAVED pack on demand and stream ONE
 * combined PDF. RLS-scoped (ctx.supabase). The stored DESCRIPTORS are re-expanded
 * against the org's CURRENT fiscal calendar here, so a pack run today reflects
 * today's dates — never the dates from when it was saved. Every figure is produced
 * by the same ledger engines as the on-screen reports, so the pack ties out.
 */
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const filename = buildExportFilename(`report-pack-${row.name}`, 'pdf');
