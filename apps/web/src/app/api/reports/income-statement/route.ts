...
export const GET = apiQueryHandler(
  ...
  async (params, ctx) => {
    ...
    // Query P&L account types with date filter
    ...
    for (const cfg of sectionConfig) {
      const accounts = Array.from(accountMap.values())
        ...
        .map((a) => {
          // Revenue: credit-normal (credits - debits)
          // COGS/OPEX/OTHER: debit-normal (debits - credits)
      ...
      for (const acct of accounts) {
        ...
        if (existing) {
          ...
          groupMap.set(acct.groupName, {
            accounts: [acct],
            totalCents: acct.amountCents,
          });
        }
      }
