...
import { parseRuleset } from '@/lib/expenses/policy-schema';

/**
 * POST /api/expenses/policy/[id]/activate — make a stored DRAFT/ARCHIVED policy the
 * single ACTIVE version for the org (archiving whatever was active). This is the
 * human-approval step that turns a compiled ruleset into enforcement.
 *
 * The stored `compiled_rules` is re-validated against the fixed schema before it
 * can go live, so a tampered blob can never enforce. RBAC: `receipts:approve`
 * (a control action). One ACTIVE per org (partial unique index is the guarantor).
 */
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  const check = parseRuleset(row.compiled_rules);
  if (!check.ok) {
  ...
  await logAction(supabase, {
    ...
    actorType: 'HUMAN',
    actorUserId: null,
    action: 'expense_policy.activate',
    subjectTable: 'expense_policies',
    subjectId: row.id,
    summary: `Expense policy v${row.version} activated`,
