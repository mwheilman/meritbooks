...
import { parseRuleset, expensePolicyRulesetSchema } from '@/lib/expenses/policy-schema';

/**
 * GET  /api/expenses/policy — list every policy version + the ACTIVE one.
 * POST /api/expenses/policy — create a policy version from a REVIEWED ruleset;
 *                             optionally activate it (archiving the current active).
 *
 * SAFETY: the ruleset is re-validated against the fixed schema here (server-side),
 * so a malformed/hand-tampered blob can never be stored or activated. Activation is
 * the human-approval step — nothing enforces until a version is ACTIVE.
 *
 * RBAC (reported: a dedicated `expense_policy` permission is the right home): read
 * gated on `receipts:view`; create/activate on `receipts:approve` (a control action).
 */
...
const createSchema = z.object({
  ...
  compiled_rules: expensePolicyRulesetSchema,
...
export async function POST(request: Request) {
  ...
  const ruleCheck = parseRuleset(parsed.data.compiled_rules);
  if (!ruleCheck.ok) {
  ...
  const { data: created, error } = await supabase
    ...
    .insert({
      org_id: orgId,
      name: parsed.data.name ?? 'Expense Policy',
      version: nextVersion,
  ...
  await logAction(supabase, {
    ...
    actorType: 'HUMAN',
    actorUserId: null,
    action: activate ? 'expense_policy.activate' : 'expense_policy.create',
    subjectTable: 'expense_policies',
    subjectId: row.id,
    summary: `Expense policy v${row.version} ${activate ? 'ACTIVATED' : 'saved as draft'}`,
