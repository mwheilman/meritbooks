...
import { Download, Loader2, ChevronDown, FileText, Sheet, Table } from 'lucide-react';
  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {busy === 'pdf' ? 'Building PDF…' : busy === 'xlsx' ? 'Building Excel…' : busy === 'csv' ? 'Building CSV…' : 'Export'}
        {!busy && <ChevronDown size={12} className="text-slate-500" />}
      </button>

      {open && !disabled && (
        <div className="absolute top-full right-0 mt-1 w-48 bg-slate-900 border border-slate-700 rounded-xl shadow-2xl z-50 py-1">
          <button onClick={() => run('xlsx')} className={clsx('w-full flex items-center gap-2 px-3 py-2 text-xs text-slate-300 hover:bg-slate-800 transition-colors')}>
            <Sheet size={13} className="text-emerald-400" /> Excel (.xlsx)
          </button>
          <button onClick={() => run('csv')} className={clsx('w-full flex items-center gap-2 px-3 py-2 text-xs text-slate-300 hover:bg-slate-800 transition-colors')}>
            <Table size={13} className="text-emerald-400" /> CSV (raw data)
          </button>
