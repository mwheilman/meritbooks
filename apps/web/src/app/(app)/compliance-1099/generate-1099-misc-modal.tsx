...
/**
 * Generate 1099-MISC flow — the MISC sibling of Generate1099Modal (which does NEC).
 * Read-only assembly (NO IRS transmit — canon §3). The user confirms payer identity,
 * reviews the assembled batch — ready records with their per-box amounts (rents /
 * royalties / other income / medical / attorney proceeds), total reportable, and the
 * FIX-FIRST blockers (missing TIN / W-9) — then downloads the filing-service e-file
 * (CSV), the branded recipient Copy B PDFs, and the IRS FIRE fixed-width file. A
 * blocked recipient is excluded from every file by construction.
 */

// Box labels — keep in sync with lib/tax/box-classify BOX_META (MISC subset).
const MISC_BOX_LABEL: Record<string, { num: string; label: string }> = {
  MISC_1: { num: '1', label: 'Rents' },
  MISC_2: { num: '2', label: 'Royalties' },
  MISC_3: { num: '3', label: 'Other income' },
  MISC_6: { num: '6', label: 'Medical / health care' },
  MISC_10: { num: '10', label: 'Attorney gross proceeds' },
};
const MISC_BOX_ORDER = ['MISC_1', 'MISC_2', 'MISC_3', 'MISC_6', 'MISC_10'];
...
interface GenSummary {
  ...
  boxTotals: Record<string, number>;
  blockedCount: number;
...
/** IRS FIRE e-file readiness (from /api/tax/1099/efile-misc?format=summary). */
interface FireInfo {
  readyCount: number;
...
export function Generate1099MiscModal({ year, onClose }: { year: number; onClose: () => void }) {
  ...
  const review = useCallback(async () => {
    ...
    const [res, fireRes] = await Promise.all([
      api.get<GenResponse>('/api/compliance/1099/generate-misc', {
        ...queryParams(year, payer),
        format: 'summary',
      }),
      api.get<FireInfo>('/api/tax/1099/efile-misc', {
        ...queryParams(year, payer),
        format: 'summary',
      }),
    ]);
    setLoading(false);
    if (res.error) {
      setError(res.error.error || 'Could not assemble 1099-MISC batch');
      return;
    }
  ...
  function download(format: 'csv' | 'pdf') {
    ...
    if (format === 'csv' && data.summary.payerTinMissing) {
    ...
    window.open(`/api/compliance/1099/generate-misc?${qs}`, '_blank');
  ...
  function downloadFire() {
    ...
    window.open(`/api/tax/1099/efile-misc?${qs}`, '_blank');
    addToast(
        ...
        <div className="max-h-[70vh] space-y-5 overflow-y-auto px-5 py-5">
          ...
          {loading && !data ? (
            ...
            <>
              ...
              {/* Per-box totals across ready records */}
              {Object.keys(summary.boxTotals || {}).length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {MISC_BOX_ORDER.filter((c) => (summary.boxTotals[c] ?? 0) > 0).map((c) => (
                    <span
                      key={c}
                      className="inline-flex items-center gap-1.5 rounded-lg border border-slate-800 bg-slate-950 px-2.5 py-1 text-[11px] text-slate-400"
                    >
              ...
              {data && data.records.length > 0 ? (
                  ...
                  <div className="overflow-hidden rounded-lg border border-slate-800">
                    <table className="w-full text-sm">
                      <tbody className="divide-y divide-slate-800/60">
                        {data.records.map((r) => (
                          <tr key={r.vendorId} className="align-top">
                            <td className="px-3 py-2">
                              <div className="text-white">{r.recipientName}</div>
                              <div className="mt-1 flex flex-wrap gap-1">
                                {MISC_BOX_ORDER.filter((c) => (r.boxAmounts[c] ?? 0) > 0).map((c) => (
                                  <span
                                    key={c}
                                    title={MISC_BOX_LABEL[c].label}
                ...
                <div className="rounded-lg border border-slate-800 bg-slate-950 p-6 text-center text-sm text-slate-500">
                  No filable 1099-MISC records yet. Mark candidates 1099-eligible and collect their W-9 / TIN, then re-assemble.
                </div>
              )}

              {excluded.length > 0 && (
                <p className="text-[11px] text-slate-600">
                  {excluded.length} candidate{excluded.length === 1 ? '' : 's'} excluded (no MISC box clears its floor —
                  $600, or $10 for royalties — or not marked 1099-eligible). Vendors whose spend is nonemployee
                  compensation belong on the 1099-NEC batch.
                </p>
              )}
            </>
          ) : null}
