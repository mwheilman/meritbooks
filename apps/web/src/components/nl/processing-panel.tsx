...
/**
 * PROCESSING lane (P1 record-JE) — the propose→approve surface for the NL bar.
 *
 * Reuses the LIVE trust spine exactly, adding NO parallel posting path:
 *   1. POST /api/journal-entries/compose  → the NL JE composer (through the Core
 *      AI gateway; writes an ai_decisions PROPOSED row). Nothing posts here.
 *   2. Human reviews / edits the balanced proposal.
 *   3. Approve → POST /api/journal-entries with decision_id → the EXISTING gated
 *      posting route (journal_entries:post RBAC, period-lock, balance, COA gates
 *      all enforced by the deterministic engine). The copilot never bypasses them.
 */

import { useState, useEffect, useMemo, useCallback } from 'react';
import { Loader2, AlertTriangle, Lightbulb, Check, HelpCircle, Sparkles } from 'lucide-react';
...
export function ProcessingPanel({ description, onPosted }: { description: string; onPosted: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch('/api/locations')
      ...
      .finally(() => alive && setLoadingLocs(false));
    return () => { alive = false; };
  }, []);

  const compose = useCallback(async () => {
    setError(''); setProposal(null); setPostError(''); setDecisionId(null); setGateway(null);
    if (!locationId) { setError('Choose a company first.'); return; }
    setComposing(true);
    try {
      ...
      if (!res.ok) {
        setError(res.status === 402 ? 'AI budget cap reached — try again next cycle or enter this manually.' : (data.error ?? 'Compose failed'));
        return;
      }
  ...
  const approveAndPost = useCallback(async () => {
    setPostError('');
  ...
  }, [totals.balanced, allResolved, locationId, entryDate, memo, lines, decisionId, onPosted]);

  return (
    <div className="space-y-3">
      <div className="flex items-center gap-2 text-2xs uppercase tracking-wider text-emerald-400">
        <Sparkles size={13} /> Processing · record a journal entry
      ...
      {!proposal && (
        <div className="grid grid-cols-3 gap-3">
          <div className="col-span-2">
            ...
            >
              <option value="">{loadingLocs ? 'Loading companies…' : 'Select a company…'}</option>
      ...
      {!proposal && (
        <div className="flex items-center gap-3">
          <button onClick={compose} disabled={composing || !locationId}
            ...
            {composing ? <Loader2 size={15} className="animate-spin" /> : <Sparkles size={15} />} Compose proposal
          </button>
        ...
        <div className="space-y-3">
          {proposal.clarifyingQuestion && (
            <div className="flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-200">
              ...
              <span><span className="font-medium">Before you approve: </span>{proposal.clarifyingQuestion}</span>
            </div>
          )}
          ...
          {proposal.unresolvedAccounts.length > 0 && (
            <div className="rounded-lg border border-rose-500/30 bg-rose-500/10 px-3 py-2 text-sm text-rose-200">
              The AI referenced account(s) not in your chart: {proposal.unresolvedAccounts.join(', ')}. Adjust before approving.
            </div>
          )}
          ...
          <div className="rounded-xl border border-slate-800 overflow-hidden">
            <div className="grid grid-cols-[1fr_100px_100px] gap-2 px-3 py-2 bg-slate-800/60 text-2xs uppercase tracking-wider text-slate-500">
              <span>Account</span><span className="text-right">Debit</span><span className="text-right">Credit</span>
            </div>
            {lines.map((l, i) => (
              <div key={i} className="grid grid-cols-[1fr_100px_100px] gap-2 px-3 py-2 items-center border-t border-slate-800">
            ...
            <div className="grid grid-cols-[1fr_100px_100px] gap-2 px-3 py-2 border-t border-slate-700 bg-slate-800/40 text-sm font-mono">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <button onClick={approveAndPost} disabled={posting || !totals.balanced || !allResolved}
              ...
              {posting ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />} Approve &amp; post
            </button>
            <span className="text-2xs text-slate-500">Confidence {(proposal.confidence * 100).toFixed(0)}%</span>
            {gateway && (
              <span className="text-2xs text-slate-500" title={`Budget: ${gateway.budgetState}`}>· metered ${(gateway.costCents / 100).toFixed(4)}</span>
            )}
