...
export async function recordDebtPayment(db: DB, args: RecordDebtEntryArgs): Promise<DebtPostResult> {
  ...
  if (line.principal_cents > 0 && !accts.liability) {
    throw new PostingError(
      'This payment reduces principal but no notes-payable / long-term-debt account is set on the loan. ' +
        'Set the liability account on the instrument first.',
    );
  }
  ...
  const interestDebitAccount = accrued ? accts.interestPayable.id : accts.interestExpense.id;
  ...
  const lines: JournalEntryLineInput[] = [];
  if (line.interest_cents > 0) {
    lines.push({ account_id: interestDebitAccount, debit_cents: line.interest_cents, credit_cents: 0, location_id: location, memo });
  }
  if (line.principal_cents > 0 && accts.liability) {
    lines.push({ account_id: accts.liability.id, debit_cents: line.principal_cents, credit_cents: 0, location_id: location, memo });
  }
  lines.push({ account_id: accts.cash.id, debit_cents: 0, credit_cents: payment, location_id: location, memo });
